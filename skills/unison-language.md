...
description: Authoritative reference for the Unison programming language — syntax, types, algebraic data types, pattern matching and `cases`, abilities/handlers, lists and tail recursion, records, Text, tests, docs, style conventions. Use when writing or reviewing Unison source; for driving UCM (scratch files, typecheck/commit, tests, refactors) use the `ucm` skill.
---
